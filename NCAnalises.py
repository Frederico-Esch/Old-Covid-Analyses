...
#%%
regressor = LinearRegression().fit(X[:, 0:1],Y)

#%%
plt.plot(X[:, 0:1],Y, c="red", label = "data")
plt.plot(X[:, 0:1],regressor.predict(X[:,0:1]), c=(0,0,1), label = "prediction")
plt.legend()
plt.show()
...
#%%
plt.plot(X[:, 0:1],Y, c="red", label = "data")
plt.plot(X[:, 0:1],regressor.predict(X[:,0:1]), c=(0,0,1), label = "linear")
...
plt.show()
#%%
score = r2_score(Y,regressor.predict(X[:,0:1]))
